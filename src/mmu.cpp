...
mmu::mmu() 
    ...
    m_memory.protect(ROM_BANK_0_OFFSET, ROM_BANK_0_LENGTH, PROT_READ);

    // 0x4000 .. 0x7fff: ROM bank 1 to ???
    m_memory.protect(ROM_BANK_N_OFFSET, ROM_BANK_N_LENGTH, PROT_READ);

    // 0x8000 .. 0x9fff: VRAM
    m_memory.protect(VRAM_OFFSET, VRAM_LENGTH, PROT_READ | PROT_WRITE);

    // 0xa000 .. 0xbfff: External RAM
    m_memory.protect(EXTERNAL_RAM_OFFSET, EXTERNAL_RAM_LENGTH, PROT_READ | PROT_WRITE);

    // 0xc000 .. 0xcfff: WRAM bank 0
    m_memory.protect(WRAM_BANK_0_OFFSET, WRAM_BANK_0_LENGTH, PROT_READ | PROT_WRITE);

    // 0xd000 .. 0xdfff: WRAM bank 1
    m_memory.protect(WRAM_BANK_1_OFFSET, WRAM_BANK_1_LENGTH, PROT_READ | PROT_WRITE);

    // 0xe000 .. 0xefff: Lower part of echo (mirror 0xc000 .. 0xcfff)
    m_memory.protect(LOWER_ECHO_OFFSET, LOWER_ECHO_LENGTH, PROT_READ | PROT_WRITE);

    // 0xf000 .. 0xffff: Multiple uses: higher part of echo, object attribute memory, hardware registers, HRAM
    m_memory.protect(MULTI_USE_OFFSET, MULTI_USE_LENGTH, PROT_READ);
}
